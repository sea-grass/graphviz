...
#define ND_dfs_mark(n) (((info_t *)(n->base.data))->dfs_mark)
...
static void ipr(long num) { printf(" %7ld", num); }

static void wcp(int nnodes, int nedges, int ncc, int ncl, char *gname,
                char *filename) {
  int i;
...
static int eval(Agraph_t *g, int root) {
  ...
  if (root) {
    aginit(g, AGNODE, "nodeinfo", sizeof(info_t), true);
  }
