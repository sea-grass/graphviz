...
- Graphviz libraries set close-on-exec on job output files being written. This
  should only affect applications using Graphviz libraries that rely on a
  fork-and-exec-ed child subprocess being able to write to the Graphviz-opened
  output file. Applications with this requirement can regain the old behavior by
  unsetting the close-on-exec flag immediately after creating a job and before
  fork-and-exec-ing. Graphviz libraries themselves are not affected by whether
...
[Unreleased (12.1.1)]: https://gitlab.com/graphviz/graphviz/compare/12.1.0...main
